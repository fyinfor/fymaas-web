export default {
  ...
  'model.form.ollamatips':
    'ヒント: 以下はfymaasで事前設定されたOllamaモデルです。希望するモデルを選択するか、右側の【{name}】入力ボックスにデプロイしたいモデルを直接入力してください。',
  'models.sort.name': '名前',
  ...
  'models.form.backendVersion.tips':
    '希望する{backend}{version}バージョンを使用するには、システムがオンライン環境で対応するバージョンをインストールする仮想環境を自動的に作成します。fymaasのアップグレード後もバックエンドバージョンは固定されます。{link}',
  ...
  'models.filter.all': 'すべて',
  'models.status.running': '稼働中',
  'models.status.deploying': 'デプロイ中',
  'models.status.stopped': '停止',
  ...
  'models.table.apiAccessInfo.proxyHint':
    '本サイトへ直接接続し、システムプロキシや VPN をオフにして、モデル接続とリクエスト転送への干渉を避けてください。',
  'models.table.apiAccessInfo.examples': 'リクエスト例',
  'models.table.apiAccessInfo.schemaTitle': 'レスポンス構造',
  'models.table.apiAccessInfo.schema.name': 'パラメータ',
  'models.table.apiAccessInfo.schema.type': '型',
  'models.table.apiAccessInfo.schema.example': '例',
  'models.table.apiAccessInfo.schema.desc': '説明',
  'models.table.apiAccessInfo.schema.id': 'この補完リクエストの一意な ID',
  'models.table.apiAccessInfo.schema.object': '返却オブジェクトの種類',
  'models.table.apiAccessInfo.schema.created': '作成時の Unix タイムスタンプ',
  'models.table.apiAccessInfo.schema.model': '実際に使用されたモデル名',
  'models.table.apiAccessInfo.schema.choices': 'モデルが生成した候補結果',
  'models.table.apiAccessInfo.schema.usage': 'このリクエストのトークン使用量',
...
// 65. 'models.form.partialoffload.tips': `When CPU offloading is enabled, fymaas will allocate CPU memory if GPU resources are insufficient. You must correctly configure the inference backend to use hybrid CPU+GPU or full CPU inference.`,
...
// 74. 'models.form.modelfile.notfound': `The model file path you specified does not exist on the fymaas server. It's recommended to place the model file at the same path on both the fymaas server and fymaas workers. This helps fymaas make better decisions.`,
