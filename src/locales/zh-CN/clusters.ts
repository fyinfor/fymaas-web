export default {
  ...
  'clusters.addworker.externalIP.tips':
    '如运行在 VPC 或私有网络时，请指定 fymaas Server 可达的节点外部地址。',
  ...
  'clusters.addworker.dataVolume': 'fymaas 数据卷',
  'clusters.addworker.dataVolume.tips': '为 fymaas 指定数据存储路径。',
  'clusters.table.ip.internal': '内',
  'clusters.table.ip.external': '外',
  'clusters.form.serverUrl.tips':
    '如果节点无法直接访问 fymaas Server，则指定一个可访问的外部 fymaas Server 地址。例如：{example}',
  ...
  'clusters.addworker.nvidiaNotes':
    'fymaas 内置推理后端依赖 <span class="bold-text">CUDA 12.8</span> 及以上版本，请确保 NVIDIA 驱动版本为 <span class="bold-text">570</span> 或以上。',
  ...
  'clusters.systemDefaultContainerRegistry.tip':
    '用于解析该集群 fymaas 镜像的默认镜像仓库。未设置时回退到服务端默认值。',
  ...
  'clusters.operatorImage.tip':
    'fymaas Operator 容器镜像的覆盖值。留空则使用服务端默认值。',
